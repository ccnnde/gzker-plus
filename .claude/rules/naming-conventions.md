...
**所有 scoped 样式中的类名必须带组件前缀，无一例外。** 前缀由组件 PascalCase 文件名转为 kebab-case 得到：`{组件名.kebab-case}-{元素名}`
...
| `ContentEditor` | `content-editor-` | `content-editor-header`  |
| `GzkSearch`     | `gzk-search-`     | `gzk-search-wrapper`     |
| `ReplyEditor`   | `reply-editor-`   | `reply-editor-container` |
| `ReplyItem`     | `reply-`          | `reply-container`        |
